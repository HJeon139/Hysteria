...
//-----------------------------------------------------------------------------
HRESULT ScanForWarnings( GDFData* pGDFDataList, D3DX10_IMAGE_INFO* pImageList, int nNumLangs, bool bQuiet, bool bWarnMissingNEU )
{
    wprintf( L"Warnings:\n" );
    ...
    for( int iLang=0; iLang<nNumLangs; iLang++ )
    {
        ...
        if ( pImageList )
        {
            ...
            {
                OutputWarning( L"%sGE thumbnail image is missing or of an unknown format.\n", strHeader );
            }
            else
            {
...
HRESULT ProcessGDF( WCHAR* strGDFPath, bool bMuteWarnings, bool bMuteGDF, bool bQuiet )
{
    ...
    if( !bMuteWarnings )
        ScanForWarnings( pGDFData, NULL, 1, bQuiet, false );
...
HRESULT ProcessBIN( WCHAR* strGDFBinPath, bool bMuteWarnings, bool bMuteGDF, bool bQuiet )
{
    ...
    for( int iLang=0; iLang<nLangs; iLang++ )
    {
        ...
        hr = gdfParse.ExtractGDFThumbnail( strGDFBinPath, &pImageList[iLang], wLang );
        if ( FAILED(hr) )
        {
    ...
    if( !bMuteWarnings )
    {
        ScanForWarnings( pGDFDataList, pImageList, nLangs, bQuiet, true );
