...
HRESULT GenerateTriCenterBuffer( ID3D10Device* pd3dDevice, LPCTSTR szMesh, UINT* puNumFaces );
                                      ...
                                      void* pUserContext )
{
    ...
    UINT uNumFaces = 0;
    V_RETURN( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, L"Crypt\\vinestart.x" ) );
    V_RETURN( GenerateTriCenterBuffer( pd3dDevice, str, &uNumFaces ) );

    // Create the seeding pipe section
    V_RETURN( CreatePipeBuffer( pd3dDevice, g_numSpawnPoints, uNumFaces ) );
...
HRESULT GenerateTriCenterBuffer( ID3D10Device* pd3dDevice, LPCTSTR szMesh, UINT* puNumFaces )
{
    ...
    if( SUCCEEDED( pRawMesh->CloneMeshFVF( D3DXMESH_32BIT, D3DFVF_XYZ, pDev9, &pMesh ) ) )
    {
        ...
        if( !pdwAdj ) return E_OUTOFMEMORY;
        pMesh->GenerateAdjacency( 1e-5f, pdwAdj );
        float* pdwAdjF = new float[pMesh->GetNumFaces() * 3 ];
        if( !pdwAdjF ) return E_OUTOFMEMORY;

        for( UINT i = 0; i < pMesh->GetNumFaces(); i++ )
        {
            pdwAdjF[ i * 3 ] = ( float )pdwAdj[ i * 3 ];
            pdwAdjF[ i * 3 + 1 ] = ( float )pdwAdj[ i * 3 + 1 ];
            pdwAdjF[ i * 3 + 2 ] = ( float )pdwAdj[ i * 3 + 2 ];
        }

        D3D10_BUFFER_DESC adesc =
        {
            pMesh->GetNumFaces() * 3 * sizeof( float ),
            D3D10_USAGE_DEFAULT,
        ...
        D3D10_SUBRESOURCE_DATA InitData;
        InitData.pSysMem = pdwAdjF;
        InitData.SysMemPitch = 0;
        V_RETURN( pd3dDevice->CreateBuffer( &adesc, &InitData, &g_pAdjBuffer ) );

        // load vertices
        D3DXVECTOR4* pVerts = new D3DXVECTOR4[ 2 * pMesh->GetNumFaces() ];
        if( !pVerts ) return E_OUTOFMEMORY;
        ...
        for( UINT i = 0; i < pMesh->GetNumFaces() * 3; i += 3 )
        {
            ...
            pVerts[ index ].x = vCenter.x;
            pVerts[ index ].y = vCenter.y;
            pVerts[ index ].z = vCenter.z;
            pVerts[ index ].w = 0.0f;
            index ++;
            pVerts[ index ].x = N.x;
            pVerts[ index ].y = N.y;
        ...
        {
            2 * pMesh->GetNumFaces() * sizeof( D3DXVECTOR4 ),
            D3D10_USAGE_DEFAULT,
            D3D10_BIND_SHADER_RESOURCE,
            0,
            0
        ...
        SAFE_DELETE_ARRAY( pdwAdj );
        SAFE_DELETE_ARRAY( pdwAdjF );
    }

    DWORD dwNumFaces = pMesh->GetNumFaces();
    *puNumFaces = dwNumFaces;
    SAFE_RELEASE( pAdj );
    SAFE_RELEASE( pMesh );
    SAFE_RELEASE( pDev9 );
    SAFE_RELEASE( pD3D9 );
    ...
    SRVDesc.Buffer.ElementWidth = 2 * dwNumFaces;
