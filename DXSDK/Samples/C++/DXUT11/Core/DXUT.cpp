...
public:
    ...
    GET_SET_ACCESSOR( IDXGIAdapter*, DXGIAdapter );
...
void DXUTUpdateDeviceSettingsWithOverrides( DXUTDeviceSettings* pDeviceSettings )
{
    ...
    if( GetDXUTState().GetOverrideHeight() != 0 )
    {
        ...
        pDeviceSettings->d3d11.sd.BufferDesc.Height = GetDXUTState().GetOverrideWidth();
    }
...
HRESULT DXUTCreate3DEnvironment11( ID3D11Device* pd3d11DeviceFromApp )
{
    ...
    if( pd3d11DeviceFromApp == NULL )
    {
        ...
        if( pNewDeviceSettings->d3d11.DriverType == D3D_DRIVER_TYPE_HARDWARE ) {
            hr = pDXGIFactory->EnumAdapters( pNewDeviceSettings->d3d11.AdapterOrdinal, &pAdapter );
            if ( FAILED( hr) ) {
                return E_FAIL;
            }
            ddt = D3D_DRIVER_TYPE_UNKNOWN;    
        }else if (pNewDeviceSettings->d3d11.DriverType == D3D_DRIVER_TYPE_WARP) {
            hr = pDXGIFactory->CreateSoftwareAdapter( LoadLibrary( L"D3D10WARP_beta.dll"), &pAdapter);
            if ( FAILED( hr) ) {
                return E_FAIL;
            }
            ddt = D3D_DRIVER_TYPE_UNKNOWN;    
        }else if (pNewDeviceSettings->d3d11.DriverType == D3D_DRIVER_TYPE_REFERENCE) {
            hr = pDXGIFactory->CreateSoftwareAdapter( LoadLibrary( L"D3D11Ref.dll"), &pAdapter);
            if ( FAILED( hr) ) {
                return E_FAIL;
            }
            ddt = D3D_DRIVER_TYPE_UNKNOWN;    
        ...
        if( SUCCEEDED( hr ) )
        {
            ...
            if( SUCCEEDED( hr ) )
            {
                //if( pNewDeviceSettings->d3d11.DriverType != D3D_DRIVER_TYPE_HARDWARE )
                {
                    ...
                    hr = pd3d11Device->QueryInterface( __uuidof( IDXGIDevice ), ( LPVOID* )&pDXGIDev );
...
HRESULT DXUTSnapDeviceSettingsToEnumDevice( DXUTDeviceSettings* pDeviceSettings, bool forceEnum,  D3D_FEATURE_LEVEL forceFL ) {
    ...
    if (bAppSupportsD3D11 && pDeviceSettings->ver == DXUT_D3D11_DEVICE ) {
        ...
        //modifySettings->d3d11.fl = D3D_FEATURE_LEVEL_11_0;
        pDeviceSettings->d3d11.Output = pDeviceSettingsCombo->Output;
        //pDeviceSettings->d3d11.PresentFlags = 0;
        //pDeviceSettings->d3d11.sd.BufferCount = pDeviceSettingsCombo->pDeviceInfo->;
        ...
        //pDeviceSettings->d3d11.sd.BufferUsage = 32;
        //pDeviceSettings->d3d11.sd.Flags = DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH ;
        //pDeviceSettings->d3d11.sd.OutputWindow = DXUTGetHWND();
    ...
    if (bAppSupportsD3D9) {
      // Find the best combination of:
        ...
        //pDeviceSettings->d3d9.BehaviorFlags = ;//64;
        pDeviceSettings->d3d9.DeviceType = pBestDeviceSettingsCombo->DeviceType;
        //pDeviceSettings->d3d9.pp.AutoDepthStencilFormat = pBestDeviceSettingsCombo->;
        ...
        //pDeviceSettings->d3d9.pp.EnableAutoDepthStencil = 1;
        //pDeviceSettings->d3d9.pp.Flags = pBestDeviceSettingsCombo->;
