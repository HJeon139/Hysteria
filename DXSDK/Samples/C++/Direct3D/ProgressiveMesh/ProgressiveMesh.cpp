                                 ...
                                 void* pUserContext )
{
    ...
    if( FAILED( hr = D3DXCleanMesh( D3DXCLEAN_SIMPLIFICATION, pMesh, ( DWORD* )pAdjacencyBuffer->GetBufferPointer(),
                                    &pTempMesh,
    ...
    {
        hr = pPMesh->ClonePMeshFVF( D3DXMESH_MANAGED | D3DXMESH_VB_SHARE, pPMesh->GetFVF(),
                                    pd3dDevice, &g_ppPMeshes[iPMesh] );
        if( FAILED( hr ) )
            goto End;

        // Trim to appropriate space
        hr = g_ppPMeshes[iPMesh]->TrimByVertices( cVerticesMin + cVerticesPerMesh * iPMesh,
    ...
    g_SampleUI.GetSlider( IDC_DETAIL )->SetRange( g_ppPMeshes[0]->GetMinVertices(), g_ppPMeshes[g_cPMeshes -
                                                  1]->GetMaxVertices() );
