...
HRESULT CScanCS::OnD3D11CreateDevice( ID3D11Device* pd3dDevice )
{
    ...
    V_RETURN( CompileShaderFromFile( L"ScanCS.hlsl", "CSScanInBucket", "cs_4_0", &pBlobCS ) );
    ...
    V_RETURN( CompileShaderFromFile( L"ScanCS.hlsl", "CSScanBucketResult", "cs_4_0", &pBlobCS ) );
    V_RETURN( pd3dDevice->CreateComputeShader( pBlobCS->GetBufferPointer(), pBlobCS->GetBufferSize(), NULL, &m_pScan2CS ) );
    SAFE_RELEASE( pBlobCS );

    V_RETURN( CompileShaderFromFile( L"ScanCS.hlsl", "CSScanAddBucketResult", "cs_4_0", &pBlobCS ) );
