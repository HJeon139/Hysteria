...
void InitApp()
{
    ...
    g_SampleUI.AddCheckBox( IDC_SHOWTESSELLATED, L"Show (t)essellated", 0, 0, 125, 22, g_bShowTessellated, 'T' );
                                     ...
                                     void* pUserContext )
{
    ...
    if ( DXUTGetDeviceSettings().d3d11.DriverType != D3D_DRIVER_TYPE_HARDWARE )
    {
    ...
    ID3DBlob* pBlob = NULL;
    V_RETURN( CompileShaderFromFile( L"Render.hlsl", "RenderVS", "vs_4_0", &pBlob ) );
    V_RETURN( pd3dDevice->CreateVertexShader( pBlob->GetBufferPointer(), pBlob->GetBufferSize(), NULL, &g_pVS ) );    
    SAFE_RELEASE( pBlob );

    V_RETURN( CompileShaderFromFile( L"Render.hlsl", "RenderBaseVS", "vs_4_0", &pBlob ) );
    ...
    V_RETURN( CompileShaderFromFile( L"Render.hlsl", "RenderPS", "ps_4_0", &pBlob ) );
